...
begin
	...
	process(RS, BRR_VOICE, STEP, SMP_A, SMP_WE, SMP_DO, KON_CNT, TDIR_ADDR, BRR_ADDR, BRR_OFFS,
			ECHO_WR_EN, TESA, ECHO_POS, EOUT)
	begin
		RAM_DO <= x"00";
			case RS is
				when RS_SRCNL =>
					if KON_CNT(BRR_VOICE) = 0 then
						RAM_A <= std_logic_vector(unsigned(TDIR_ADDR) + 2 + 0);
					else
						RAM_A <= std_logic_vector(unsigned(TDIR_ADDR) + 0 + 0);
					end if;
				...
				when RS_SRCNH =>
					if KON_CNT(BRR_VOICE) = 0 then
						RAM_A <= std_logic_vector(unsigned(TDIR_ADDR) + 2 + 1);
					else
						RAM_A <= std_logic_vector(unsigned(TDIR_ADDR) + 0 + 1);
					end if;
				...
				when RS_ECHORDL =>
					if STEP = 22 then
						RAM_A <= std_logic_vector(ECHO_ADDR + 0 + 0);
					else
						RAM_A <= std_logic_vector(ECHO_ADDR + 2 + 0);
					end if;
					RAM_WE <= '0';
					RAM_OE <= '1';
					RAM_CE <= '1';
				...
				when RS_ECHORDH =>
					if STEP = 22 then
						RAM_A <= std_logic_vector(ECHO_ADDR + 0 + 1);
					else
						RAM_A <= std_logic_vector(ECHO_ADDR + 2 + 1);
					end if;
					RAM_WE <= '0';
					RAM_OE <= '1';
					RAM_CE <= '1';
				...
				when RS_ECHOWRL =>
					if STEP = 29 then
						RAM_A <= std_logic_vector(ECHO_ADDR + 0 + 0);
						RAM_DO <= std_logic_vector(EOUT(0)(7 downto 0));
					else
						RAM_A <= std_logic_vector(ECHO_ADDR + 2 + 0);
						RAM_DO <= std_logic_vector(EOUT(1)(7 downto 0));
					end if;
				...
				when RS_ECHOWRH =>
					if STEP = 29 then
						RAM_A <= std_logic_vector(ECHO_ADDR + 0 + 1);
						RAM_DO <= std_logic_vector(EOUT(0)(15 downto 8));
					else
						RAM_A <= std_logic_vector(ECHO_ADDR + 2 + 1);
						RAM_DO <= std_logic_vector(EOUT(1)(15 downto 8));
					end if;
